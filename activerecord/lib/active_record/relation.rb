...
module ActiveRecord
  ...
  class Relation
    ...
    def uniform_update_all(updates, record_timestamps: nil)
      ...
      unless group_values.empty? && having_clause.empty?
        raise ArgumentError, "No support to update grouped relations"
      end

      model.with_connection do |c|
        values_table, join_conditions, values = UniformUpdateAll.new(self, c, updates, record_timestamps:).to_arel
        ...
        c.update(stmt, "#{model} Uniform Update All").tap { reset }
      end
    end
