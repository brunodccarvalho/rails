...
module ActiveRecord
  class UpdateAll # :nodoc:
    ...
    def initialize(relation, connection, updates, record_timestamps: nil)
      ...
      if @updates.empty?
        @read_keys, @write_keys = Set.new, Set.new
      else
        resolve_attribute_aliases
        @read_keys = @updates.first.first.keys.to_set
        @write_keys = @updates.first.last.keys.to_set
      end
    end
    ...
    def to_arel
      ...
      values_table = Arel::Nodes::ValuesTable.new(:__active_record_uua_temp, rows)
      ...
      set_assignments = write_keys.map.with_index do |key, index|
        [model.arel_table[key], cast_value(key, values_table[index + read_keys.size])]
      end

      set_assignments += timestamp_keys.map do |key|
      ...
      # table = model.arel_table.join(values_table).on(*join_conditions).ast.cores.first.source
      # Arel::UpdateManager.new.table(table).set(set_assignments).ast.to_sql(model)
    end
    ...
    private
      ...
      def verify_attributes(conditions, writes)
        if read_keys != conditions.keys.to_set
          raise ArgumentError, "All objects being updated must have the same condition keys"
        end
        if write_keys != writes.keys.to_set
          raise ArgumentError, "All objects being updated must have the same write keys"
        end
      end
      ...
      def normalize_updates(updates)
        if updates.is_a?(Hash)
          if model.composite_primary_key?
            updates.map { |id, writes| [model.primary_key.zip(id).to_h, writes] }
          elsif updates.keys.first.is_a?(Array)
            updates.map { |id, writes| [{ model.primary_key => id.fetch(0) }, writes] }
          else
            updates.map { |id, writes| [{ model.primary_key => id }, writes] }
          end
        else
          updates
        end.map do |conditions, writes|
          [conditions.stringify_keys, writes.stringify_keys]
        end
      end

      def map_key_with_value
        updates.map do |conditions, writes|
          verify_attributes(conditions, writes)

          condition_values = read_keys.map { |key| yield key, conditions[key] }
          write_values = write_keys.map { |key| yield key, writes[key] }
      ...
      def resolve_attribute_aliases
        ...
        @updates.map! do |conditions, update|
          [conditions.transform_keys { |attribute| resolve_attribute_alias(attribute) },
           update.transform_keys { |attribute| resolve_attribute_alias(attribute) }]
        end
      end
      ...
      def cast_value(name, value)
        # Unlike INSERTs, the entry in the VALUES table is untyped so it may need to be cast
        if connection.requires_cast_for_untyped_comparison?(model.columns_hash[name])
          model.arel_table.cast(value, model.columns_hash[name].sql_type)
        else
          value
        end
      end
