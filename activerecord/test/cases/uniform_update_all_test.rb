...
class UniformUpdateAllTest < ActiveRecord::TestCase
  ...
  def test_uniform_update_all_supports_typecasting_for_jsons
    ...
    puts Admin::User.type_for_attribute('json_options').inspect
  ...
  def test_uniform_update_all_with_group_by
    ...
    high_post = Post.joins(:comments).group("posts.id").having("count(comments.id) < #{minimum_comments_count}").first.id
    low_post = Post.joins(:comments).group("posts.id").having("count(comments.id) >= #{minimum_comments_count}").first.id
    ...
    assert_equal "ig", Post.find(high_post).title
    assert_not_equal "ig", Post.find(low_post).title
  end
  ...
  def test_uniform_update_all_with_nil_composite_primary_key
    affected_rows = Cpk::Car.uniform_update_all [
      ...
      [{ make: "Honda",  model: nil },     { year: 1972 }]
    ]

    assert_equal 1, affected_rows
    assert_equal 1972, Cpk::Car.find(["Honda", "Civic"]).year
  end
