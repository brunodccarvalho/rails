...
class UpdateInBulkTest < ActiveRecord::TestCase
  fixtures :'admin/users', :books, :comments, :cpk_cars, :developers, :posts,
    :taggings, :tags, :people, :pets, :toys
  ...
  def setup
    ...
    skip unless ActiveRecord::Base.lease_connection.supports_values_tables?
  end
  ...
  def test_update_in_bulk_uniform
    Book.update_in_bulk [
  ...
  def test_update_in_bulk_by_primary_key_uniform
    Book.update_in_bulk({
  ...
  def test_update_in_bulk_by_composite_primary_key_uniform
  ...
  def test_update_in_bulk_without_values
    assert_no_queries do
      Book.update_in_bulk({ 1 => {} })
    end
    assert_no_queries do
      Book.update_in_bulk [[{ id: 1 }, {}]]
    end
  end
  ...
  def test_update_in_bulk_performs_a_single_update
    affected_rows = Cpk::Car.update_in_bulk({
  ...
  def test_update_in_bulk_with_empty_updates
    assert_no_queries do
  ...
  def test_update_in_bulk_does_not_support_referential_arel_sql_in_values
    assert_raises(ActiveRecord::StatementInvalid) do
      Book.update_in_bulk [[{ name: "Jamis" }, { name: Arel.sql("UPPER(name)") }]]
    end
  end
  ...
  def test_update_in_bulk_resets_relation
    ...
    assert_changes "audit_logs.loaded?", from: true, to: false do
      audit_logs.update_in_bulk({ 1 => { message: "event" } })
    end
  end
  ...
  def test_update_in_bulk_with_scope
    tag = Tag.first # posts: [1, 2]
    Post.tagged_with(tag.id).update_in_bulk({
      1 => { title: "first" },
      2 => { title: "second" }
    })
